...
<form name="form1" method="post" action="<?=base_url()?>index.php/customer/update" enctype="multipart/form-data"/>
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
       ...
       <td colspan="2">
         <input type="text" name="customer_name" class="form-control"  placeholder="客户姓名" value="<?=$info['customer_name']?>"> 
       </td>
  </tr>
      <tr>
        <td class="tableleft">手机</td>       
       <td colspan="2">
         <input type="text" name="mobile" class="form-control"  placeholder="手机" value="<?=$info['mobile']?>"> 
       </td>
  </tr>
  ...
  <tr>
       ...
       <td colspan="2">
           <select data-placeholder="Choose a Country..." class="chosen-select" style="width:350px;" tabindex="2" name="sale_id" >
                ...
                ?>
                <option value="<?=$v2['id']?>" <?php if($info['sale_id'] == $v2['id']){ ?> selected <?php } ?>><?=$v2['username']?>-<?=$v2['realname']?></option>
                <?php
                  }
                ?>
            </select> 
